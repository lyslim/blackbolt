...
$mo.namespace('components.workforce.resource_grid');

components.workforce.resource_grid = function()
{
    var id = $(this).attr('id');
    if (id == '') { return this; }
...
//        this.set_signals();
    ...
    panel.prototype = {
        container      : 'resource_grid',
        store_url      : url_for('workforce/resource.json'),
 ...
 //       order_url      : url_for('projects/' + p_cd + '/items/{:id}/item_order_update.json'),
		...
		create_grid: function()
		{
            var colModel   = new Ext.grid.ColumnModel([
                {header: 'Staff', width: 200, menuDisabled: true, sortable: false, locked: true, dataIndex: 'item_name' },
                {header: '', menuDisabled: true, sortable: false, locked: true, dataIndex: 'level', id:'level' }
            ]);
...
/*
        create_store: function()
        {
            this.store = new Ext.i3.data.GroupJsonStore({
                ...
                groups : [
                    {name: 'user_name',       mapping:'item_name'}
                ],
                copyfields : [
                    {name: 'project_id'}
                ]
...
/*
        ...
        {
            var colModel   = new Ext.grid.ColumnModel([
                {header: 'Staff', width: 200, menuDisabled: true, sortable: false, locked: true, dataIndex: 'item_name', renderer: this.onItemNameRenderer.createDelegate(this) },
...
/*
		...
		fnCalOccupyRate: function(grid, rec, rowIndex, store){
			var nextrec;
			var oRate = 0;
			if (rec.get('isgroup')) {
				rowIndex++;
				while(!grid.store.getAt(rowIndex++).isgroup) {
					nextrec = grid.store.getAt(rowIndex++) ;
					oRate += parseFloat(nextrec.task_kbn);
				}
			}	 
			return oRate;
		},
...
*/
        ...
        onChangeProject : function(){
            ...
            var start_date      = '2011-02-22';
            var end_date        = '2011-03-22';
...
$.fn.workforce_resource_grid = components.workforce.resource_grid;
